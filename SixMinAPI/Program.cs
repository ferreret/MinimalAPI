...
app.MapPost("api/v1/commands", async (ICommandRepo repo,
                                      IMapper mapper,
                                      CommandCreateDto cmdCreateCto) =>
{
    var commandModel = mapper.Map<Command>(cmdCreateCto);
    await repo.CreateCommand(commandModel);
    await repo.SaveChanges();

    var cmdReadCto = mapper.Map<CommandReadDto>(commandModel);
    return Results.Created($"api/v1/commands/{cmdReadCto.Id}", cmdReadCto);
});
